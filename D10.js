...
let arrskills = me.skills
arrskills.push("java script")
console.log(me)
...
console.log(deleteOne("Arrivederci",true))
console.log(deleteOne("Arrivederci",false))
...
console.log(isThisAnEmail("lorellamarino@gmail.com"))
...
function howManyDays(data){
  let oggi = new Date()
  let datapassata = new Date(data)
  let diff = oggi - datapassata
  let diffgiorni = Math.floor(diff / (1000 * 60 * 60 * 24))
  return diffgiorni
}

console.log(howManyDays("2024-11-18"))
...
/* ESERCIZIO 11
  ...
  deve ritornare l'oggetto fornito dopo aver eliminato in esso la proprietà chiamata
...
let obj1 = {name: "Lorella", surname: "Marino", age: 24}
...
function newestMovie (array) {
  let ordinefilm = movies.sort((a, b) => b.Year - a.Year)
  return ordinefilm[0];
}
...
function searchByTitle(array, stringa) {
  return array.filter (movies => movies.Title.includes(stringa));
}
...
function searchAndDivide(array,stringa){
  ...
  array.forEach(movie => {
    if (movie.Title.includes(stringa)) {
      return obj.match.push(movie)
    } else {
      return obj.unmatch.push(movie)
    }
  });
  return obj  
}
...
function backgroung() {
  ...
  return link.style.backgroungcolor = "red"
}
...
function svuotalista() {
  let mylist = document.getElementById("myList")
  return mylist.removeChild(mylist.firstChild)
}
